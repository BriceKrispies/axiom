...
When a fix genuinely needs the arms to differ, the sanctioned mechanism is a
`RenderCapability` with a declared `CapabilityDegradation` — or, at the app tier, the app
reading `WindowingApi::observe_bound_backend()` (as `apps/burnt-rubber` does for its
near-field paint window). Never two divergent scenes, and never a lesser frame handed to a
lesser backend.
