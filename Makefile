...
.PHONY: gallery gallery-build gallery-serve gallery-fast gallery-fast-build \
	netplay netplay-build netplay-server netplay-dotnet relay doom-hot \
...
help:
	...
	@echo ""
	@echo "  DOOM live level hot-reload:"
	@echo "  make doom-hot       Build the fast gallery + serve DOOM with live level hot-reload at http://localhost:8080/doom/"
	@echo "  (edit apps/axiom-gallery/src/doom/level.axiom and save to reload the level live.)"
	@echo ""
	@echo "  Agent drivers (native, feature-gated bins of the gallery crate):"
	@echo "  make agent          DOOM headless agent server (JSON over HTTP on :7878)"
...
# --- DOOM live level hot-reload ---

# Serve DOOM with LIVE LEVEL HOT-RELOAD. Builds the fast gallery into dist/ first
# (so the bundle + DOOM page exist), then the axiom-dev-reload dev server serves dist/
...
# http://localhost:8080/doom/ and edit apps/axiom-gallery/src/doom/level.axiom.
doom-hot: gallery-fast-build
	@echo Serving DOOM with hot-reload at http://localhost:8080/doom/ - edit apps/axiom-gallery/src/doom/level.axiom and save.
...
agent:
	cargo run -p axiom-gallery --features doom-agent --bin doom-agent

# Same, plus an offscreen wgpu render so `{"render":true}` returns a PNG path.
agent-render:
	cargo run -p axiom-gallery --features doom-agent-render --bin doom-agent
...
agent-bridge:
	cargo run -p axiom-gallery --features doom-agent --bin doom-agent -- --bridge
