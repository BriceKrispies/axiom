...
//! is shape-agnostic, so this browser demo owns the geometry it draws (the
//! headless demo owns its own copy — apps may not depend on one another).
//! This builds the unit cube — 24 vertices (one set of per-face normals), 36
//! indices, white vertex colour — in the
