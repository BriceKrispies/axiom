...
use axiom_demo_rotating_cube_browser as app;
...
fn load_package() -> PackageManifest {
    let text = std::fs::read_to_string(manifest_dir().join("axiom.app.toml"))
...
fn load_scene(entry: &str) -> Scene {
    let text =
        std::fs::read_to_string(manifest_dir().join(entry)).expect("entry scene file is present");
