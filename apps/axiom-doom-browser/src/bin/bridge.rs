...
//! live browser DOOM (connected over a WebSocket) and return the browser's
...
use axiom_doom_browser::agent::Action;
...
pub fn run(http_addr: &str, ws_addr: &str) {
    ...
    println!("axiom-doom agent BRIDGE: http://{http_addr}  (browser ws://{ws_addr})");
