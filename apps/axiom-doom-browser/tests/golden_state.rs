//! Phase 1 — stored golden capture for the deterministic DOOM game state.
//!
...
//! scenario through `DoomGame` and stores the canonical `write_state()` bytes
...
fn assert_golden(name: &str, actual: &[u8]) {
    ...
    match std::fs::read(&path).ok() {
        Some(expected) if !force => assert_eq!(
            actual,
            expected.as_slice(),
            "golden mismatch for `{name}` ({} vs {} bytes): DOOM output drifted. \
...
#[test]
fn golden_doom_state_sequence() {
    let (states, _) = run_canonical();
    assert_golden("doom_state_sequence", &states);
}

#[test]
fn golden_doom_hud_sequence() {
    let (_, huds) = run_canonical();
    assert_golden("doom_hud_sequence", &huds);
}
