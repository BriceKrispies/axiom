/*
 * The browser boot harness for the TS-only DOOM app.
 *
...
const boot_ = async (): Promise<void> => {
  ...
  const load = async (version: number): Promise<void> => {
    ...
    const mod = (await import(`/dist/game.js?v=${version}`)) as DoomModule;
    ...
    teardown = boot(
      ...
      // DOOM's "step the sim once per frame" loop (no real-time accumulator).
