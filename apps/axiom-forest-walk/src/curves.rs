//! Scalar and small-array interpolation curves shared across this app: plain
//! linear interpolation and the two smoothstep forms used by the visual-target
//! diorama builder (`visual_target/build.rs`) and the scatter placer
//! (`visual_target/scatter.rs`). Copied with the diorama builder from the
//! gallery growth demo in the gallery de-merge.
//!
...
//! path). Promoting these plain scalar helpers into `axiom-math` as *free*
//! functions (so the whole engine can share them) remains a candidate follow-up,
//! out of scope for the de-merge.
