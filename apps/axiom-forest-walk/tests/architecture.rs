//! Architecture-boundary tests for the `axiom-forest-walk` app.
//!
//! The app is a browser demo (wasm-bindgen/web-sys are legal here — it is a
//! composition leaf, not spine code), but its diorama builder is deterministic:
//! no wall-clock time, no ambient randomness. These tests also pin the app.toml
//! manifest to the layers/modules the crate actually consumes and prove no
...
fn app_toml_exists_and_lists_only_consumed_layers_and_modules() {
    ...
    // The layers the diorama builder + walk consume.
    for required in [
        "\"kernel\"",
        "\"math\"",
        "\"host\"",
        ...
        assert!(
            ...
            "app.toml lists the consumed dependency {required}"
        );
    }
...
fn no_wall_clock_or_randomness() {
    assert_absent_in_app(
        ...
        "the forest is deterministic: no wall-clock time, no ambient randomness",
    );
}
...
fn no_placeholder_macros() {
    assert_absent_in_app(
        &["todo!", "unimplemented!"],
        "no placeholder architecture in the forest walk",
    );
}

#[test]
fn no_illegal_engine_imports() {
    // The app may import only the axiom crates its app.toml declares (+ std,
    // serde/toml, and the wasm glue crates).
    let allowed = [
        "axiom_kernel",
        "axiom_math",
        "axiom_host",
    ...
    assert!(
        illegal.is_empty(),
        "app may only import the axiom crates declared in app.toml:\n{}",
        illegal.join("\n")
    );
}
...
fn no_junk_drawer_modules() {
    for path in app_sources() {
        ...
        for banned in ["utils", "helpers", "common", "misc"] {
            assert_ne!(name, banned, "no `{banned}` module in the forest walk");
        }
    }
}
...
fn no_layer_or_module_depends_on_this_app() {
    let mut hits = scan_raw(repo_root().join("crates"), &["axiom_forest_walk"]);
    ...
    assert!(
        hits.is_empty(),
        "no layer/module may depend on the forest-walk app:\n{}",
        hits.join("\n")
    );
}
