...
apps/axiom-gallery/
  ...
  src/
    ...
    <demo>/           # one demo's source (its old crate, nested as a module)
      bin/            # native, feature-gated agent drivers (doom, growth)
...
- **Entries are namespaced.** Every demo's wasm entry was `start`; in one crate
  ...
  (`doom_start`, `quintet_start`, …). Growth has no `start` (it exports
...
- **Native agent drivers survive as feature-gated bins**: `doom-agent`
  (feature `doom-agent`), `growth-agent` (feature `growth-agent`), and the
