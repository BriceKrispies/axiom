# Axiom DOOM — agent bridge protocol

A small JSON-over-HTTP protocol that lets an external agent (or any client)
**drive the real DOOM game and read back what it does** — locally headless, or
...
cargo run -p axiom-doom-browser --features agent --bin agent
# A+images — same, plus an offscreen-rendered PNG per `render` request
cargo run -p axiom-doom-browser --features agent-render --bin agent
# B. bridge — relay to a live browser opened with ?agent=ws://127.0.0.1:7879
cargo run -p axiom-doom-browser --features agent --bin agent -- --bridge
```
...
Browser bridge: run `--bridge`, open `…/demo.html?id=doom&agent=ws://127.0.0.1:7879`
