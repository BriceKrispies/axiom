...
pub struct StepCommands {
    ...
    /// returned [`Entity`] back via [`DoomGame::set_enemy_entity`].
...
impl DoomGame {
    ...
    /// `level_setup` marked `Player(i)`, so after `build_doom_app` (or a fork's
    ...
    fn live_step(&mut self, intent: Intent, space: &dyn DoomSpace) -> StepCommands {
    ...
    fn move_player(&mut self, forward: f32, strafe: f32, space: &dyn DoomSpace) -> Vec3 {
    ...
    fn wall_blocked(&self, space: &dyn DoomSpace, x: f32, z: f32) -> bool {
    ...
    fn fire(&mut self, firing: bool, space: &dyn DoomSpace) -> Option<Entity> {
    ...
    fn fire_shot(&mut self, space: &dyn DoomSpace) -> Option<Entity> {
        let tun = self.level.tun;
    ...
    fn update_enemies(&mut self, space: &dyn DoomSpace, respawning: bool) -> Vec<PlayerInput> {
    ...
    fn any_enemy_in_contact(&self, space: &dyn DoomSpace) -> bool {
        let tun = self.level.tun;
    ...
    fn respawn(&mut self, space: &dyn DoomSpace) -> StepCommands {
        let tun = self.level.tun;
...
pub fn build_doom_app(doc: &LevelDoc) -> (RunningApp, DoomAssets) {
    let sink: Rc<RefCell<Option<DoomAssets>>> = Rc::new(RefCell::new(None));
    let app = App::new()
        .window(
            Window::new(960, 600)
...
/// [`DoomAssets`] (the new scene's cube mesh + enemy material), since re-authoring
/// registers new handles the caller must keep for runtime re-spawns.
pub fn reload_doom(running: &mut RunningApp, doc: &LevelDoc) -> DoomAssets {
    let sink: Rc<RefCell<Option<DoomAssets>>> = Rc::new(RefCell::new(None));
    running.set_clear_color(color_of(doc.colors.clear).to_array());
    running.reauthor(level_setup(doc.clone(), sink.clone()));
    // Copy the captured assets out (DoomAssets: Copy), dropping the borrow first.
...
/// via [`DoomGame::set_enemy_entity`]. Centralizes the enemy prototype (a cube
/// mesh, the enemy material, bounds, and a contact shadow), built from the
/// app-held `DoomAssets` handles, so the browser, the agent bridge, and the tests
...
fn level_setup(
    doc: LevelDoc,
    assets_sink: Rc<RefCell<Option<DoomAssets>>>,
    ...
    move |world, meshes, materials| {
        ...
        // builder, which holds them in DoomAssets for `apply_lifecycle` to spawn
        // fresh enemies against the same registered assets.
        *assets_sink.borrow_mut() = Some(DoomAssets { cube, enemy });
...
#[cfg(all(feature = "doom-agent", not(target_arch = "wasm32")))]
pub mod agent;
...
mod tests {
    ...
    fn game_and_app() -> (DoomGame, RunningApp, DoomAssets) {
        let doc = LevelDoc::default();
        let mut game = DoomGame::from_level(&doc);
        let (app, assets) = build_doom_app(&doc);
    ...
    fn a_longer_wall_run_in_the_document_adds_wall_renderables() {
        ...
        // + one cube per enemy).
        let (app, _assets) = build_doom_app(&doc);
        assert_eq!(
    ...
    fn reload_doom_reauthors_the_running_scene() {
        ...
        let (mut running, _assets) = build_doom_app(&LevelDoc::default());
        let before = running.renderable_count();
        let _ = running.tick(0);
        let tiny = LevelDoc::parse("[map]\n#####\n#S.E#\n#####\n");
        let _assets = reload_doom(&mut running, &tiny);
    ...
    fn the_first_frame_draws_every_renderable_and_runs_deterministically() {
        let (mut a, _assets_a) = build_doom_app(&LevelDoc::default());
        let (mut b, _assets_b) = build_doom_app(&LevelDoc::default());
        let fa = a.tick(0);
