//! Live, game-agnostic perception for the DOOM agent — the app-side **sense
//! adapter**. Native + `agent` feature only, like [`crate::doom::agent`].
...
//! per-game part**: it knows how to cast a probe against *DOOM's* world (the
//! engine scene, via [`RunningApp::raycast_hit`]), how to enumerate DOOM's enemy
...
use crate::doom::agent::{
    control_code_of, doom_drive_tick, intent_of_control_code, AGENT_RAW_ID, FIXED_DELTA_NANOS,
};
use crate::doom::{build_doom_app, level::LevelDoc, DoomAssets, DoomGame, Intent};
...
/// The player's forward direction from its yaw, in DOOM's convention (yaw 0 faces
/// `-Z`; matches the hitscan aim in [`DoomGame::fire_shot`]).
...
/// and return the lowered DOOM [`Intent`]. A scripted brain encodes the priority
...
/// replaces).
fn tag_enemies(app: &mut RunningApp, game: &DoomGame) {
...
/// A live perception-driven DOOM session: it perceives the world each tick and
/// lets the agent act on what it sees, through the real `DoomGame` → engine path.
#[derive(Debug)]
pub struct DoomPerceiver {
    game: DoomGame,
    app: RunningApp,
    assets: DoomAssets,
    tick: u64,
...
impl DoomPerceiver {
    /// Start a fresh DOOM game, bind the enemies to their engine nodes, and tag
    /// them so the agent's hits classify entity-natively from tick 0.
    pub fn new() -> Self {
        let mut game = DoomGame::new();
        let (mut app, assets) = build_doom_app(&LevelDoc::default());
...
impl Default for DoomPerceiver {
    fn default() -> Self {
        DoomPerceiver::new()
    }
}
