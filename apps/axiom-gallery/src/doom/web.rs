...
pub fn doom_start() {
    ...
    let (running_app, initial_assets) = build_doom_app(&doc);
        ...
        move |frame_index: u64, fps_milli: u32, frame_micros: u32| {
            ...
            if let Some(text) = pending_level.borrow_mut().take() {
                ...
                *game.borrow_mut() = DoomGame::from_level(&new_doc);
                let new_assets = reload_doom(&mut running.borrow_mut(), &new_doc);
            ...
            // DOOM steps the sim exactly once per frame, so `sim ticks` is 1.
...
/// Movement action ids — the neutral `u32`s the doom [`Keymap`] resolves to, one
...
/// The doom movement bindings as an interface-layer [`Keymap`]. Built from
...
/// Is this DOOM running as one pane of the gallery's backend-comparison triptych
