...
pub fn sample_macro_continuous(atlas: &PlanetSurfaceAtlas, dir: [f32; 3]) -> f32 {
    ...
    let primary = crate::growth::sampler::locate_region(atlas, v);
...
/// composited with a [`crate::growth::vista::MountainVistaPlan`].
///
...
/// passed through [`crate::growth::vista::vista_height_m`], which flattens the spawn
...
pub fn sample_height_m_lod_vista(
    ...
    z_m: f32,
    min_feature_m: f32,
    vista: Option<&crate::growth::vista::MountainVistaPlan>,
) -> f32 {
    ...
    match vista {
        Some(plan) => crate::growth::vista::vista_height_m(plan, base, x_m, z_m),
        None => base,
    }
}
...
mod tests {
    use super::*;
    use crate::growth::model_planet::{PlanetSurfaceAtlas, RegionGraph};
    ...
    fn anchor_prefers_land() {
        ...
        let r = crate::growth::sampler::locate_region(&atlas, d);
