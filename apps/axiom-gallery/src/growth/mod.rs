...
// Native first-person ground-walk sim, shared by the wasm viewer and the
// headless agent driver.
pub mod ground;

// Data-driven semantic world tags (the agent's "nouns"), fed to
// axiom-introspect so a directive can be resolved by name.
#[cfg(feature = "growth-agent")]
pub mod world_tags;

// The reusable axiom-agent driver: walks the ground sim through
// axiom-agent-harness, e.g. holding forward to the summit.
#[cfg(feature = "growth-agent")]
pub mod agent;

// Live, game-agnostic perception: the heightfield sense adapter that casts the
// reusable `axiom-perception` ray-fan against the terrain (by marching the
// sampler) — the same module the retro FPS agent casts against its scene.
#[cfg(feature = "growth-agent")]
pub mod perception;
...
// In-browser terrain viewer (wasm32 only): the live wgpu/first-person
// presentation arm. Never compiled on native, so the deterministic worldgen
...
pub struct Growth {
    ...
    /// Determinism hash of the generated globe.
    pub world_hash: u64,
    /// Stage log from generation.
...
impl Growth {
    ...
    pub fn generate(seed_str: &str, preset: PlanetPreset, site_target: u32) -> Self {
        ...
        // Validate dual region rings before hydrology. Logged (not aborted) so
        // generation is observable; the QA gate test asserts validity on
        // reference configs.
        ...
        // The transient globe is dropped here; only the atlas is retained.
    ...
    /// Overworld query at a unit direction.
    pub fn sample_surface(&self, dir: Vec3) -> SurfaceSample {
        sampler::sample_surface(&self.atlas, dir)
    }

    /// Leave the overworld and begin streaming the game world.
