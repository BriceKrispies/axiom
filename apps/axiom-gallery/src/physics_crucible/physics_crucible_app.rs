...
//! the app-owned value types in [`crate::physics_crucible::crucible_report`] ([`BodyState`],
...
use crate::physics_crucible::crucible_camera;
use crate::physics_crucible::crucible_report::{BodyState, ContactInfo, CrucibleReport, StepCounts};
use crate::physics_crucible::crucible_scenario::{
...
use crate::physics_crucible::debug_geometry::{self, palette, CrucibleMesh, RenderInstance};
use crate::physics_crucible::physics_to_render;
...
impl Crucible {
    ...
    pub fn report(&self) -> CrucibleReport {
        let (center, radius) = crate::physics_crucible::query_bay::probe_world();
...
fn crucible_instances() -> Vec<RenderInstance> {
    let stations = crate::physics_crucible::all_stations();
...
mod tests {
    ...
    fn the_live_room_builds_and_renders_draws() {
        let mut world = CrucibleWorld::new();
        for station in crate::physics_crucible::all_stations() {
            station.populate(&mut world);
        }
        let instances = live_instances(&world);
        assert!(!instances.is_empty(), "the live room has renderables");
        let (eye, target) = crate::physics_crucible::crucible_camera::orbit(0);
