...
//! mouse-look for the live browser player, and `RetroFpsGame::teleport` (used by
...
/// The app's retro FPS control bitmask: the meaning the app assigns to a neutral
/// `ActionIntent` `control_code`. `axiom-agent` carries the `u32` opaquely; this
/// is the app-side convention that packs retro FPS's discrete held controls into it,
...
/// Every retro FPS control bit, in stable order — used to split a combined control
...
/// the `RuntimeStep` that drives one decision; it does not affect the retro FPS
...
/// Translate an agent action's `keys` + `fire` into a retro FPS [`Intent`]'s discrete
...
/// Pack a retro FPS [`Intent`]'s discrete held controls into the neutral control-code
...
/// Lower a neutral control-code bitmask back into the concrete retro FPS [`Intent`]
...
fn decide_intent(px: f32, pz: f32, yaw: f32, hud: Hud, control_code: u32, tick: u64) -> Intent {
    ...
    // Observe: translate the live retro FPS state into a neutral observation.
...
/// Start a fresh retro FPS game + engine app and advance one idle tick (tick 0), so
...
/// Drive one real retro FPS frame from a concrete [`Intent`]: step the game against
...
pub(crate) fn retro_fps_drive_tick(
...
fn retro_fps_observation(game: &RetroFpsGame, last: &FrameOutcome, tick: u64) -> Observation {
    let hud = game.hud();
...
/// A live agent session: the real retro FPS game plus its engine app, driven one
/// action at a time through `axiom-agent`. Its authority is identical to the
/// browser player's — the same `RetroFpsGame::step` → `tick_with_controls` path — and
...
impl AgentSession {
    ...
    pub fn new() -> Self {
        let (game, app, assets, last) = retro_fps_session_start();
        AgentSession {
            game,
            app,
            assets,
    ...
    pub fn observe(&self) -> Observation {
        retro_fps_observation(&self.game, &self.last, self.tick)
    }
