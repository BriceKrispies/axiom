//! Golden regression for the DOOM agent (driven through `axiom-agent`).
//!
//! Locks what the agent does over a fixed action script against a committed
//! **golden snapshot** (`tests/doom/golden/doom_agent_play.txt`): the per-step HUD,
...
//! `AXIOM_UPDATE_GOLDEN=1 cargo test -p axiom-doom-browser --features agent
//! --test agent_golden`.
...
fn golden_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/doom/golden/doom_agent_play.txt")
}
...
fn the_agent_reproduces_the_golden_snapshot() {
    ...
    eprintln!("DOOM agent (axiom-agent-driven): {} steps", play.len());
    eprintln!(
