...
use crate::ground::{CaptureInputs, GroundSim, MOVE_SPEED};
use crate::presets::PlanetPreset;
use crate::world_tags;
...
impl AgentSession {
    ...
    /// same module the retro FPS agent uses; growth only supplies the heightfield probe
    /// (see [`crate::perception`]).
    pub fn sight(&self) -> crate::perception::Sight {
        crate::perception::sense_sim(&self.sim)
    }
