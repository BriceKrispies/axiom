...
//! retro FPS uses, but marches the terrain sampler ([`GroundSim::ground_abs_at`])
...
//! the view cone the same way retro FPS's enemies are.
...
mod tests {
    ...
    fn perception_is_deterministic() {
        let run = || {
            ...
            for _ in 0..30 {
                s.step(&crate::agent::Action::seek());
