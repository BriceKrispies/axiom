/*
 * meshgen.ts — procedural geometry the SDK's primitive set (box / sphere / cylinder)
 * can't express: the hoop rim + net gather-ring, both tori. SDK-free (returns a plain
 * `MeshData`-shaped object of `{positions, normals, indices}`), so it stays testable and
 * `scene.ts` hands the result straight to `createMeshData`.
 */
...
/**
 * A torus lying in the XZ plane (its axis is +Y) — the shape of a basketball rim and
 * the net's gather ring. `ringR` is the big radius, `tubeR` the tube thickness.
 */
