...
retro FPS browser app uses for live game logic. It is interactive: **▲ / Space / K**
...
- **Live (browser, `src/web.rs`).** The wasm arm owns the render loop directly via
  ...
  camera orbits. This is the same pattern the retro FPS browser app uses to drive live
