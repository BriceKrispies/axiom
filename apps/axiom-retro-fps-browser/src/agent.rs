//! The native agent bridge: drive the real retro FPS game from JSON and read back
...
/// `RetroFpsGame::step` → `tick_with_controls` path as `web.rs`.
...
impl AgentSession {
    ...
    pub fn new() -> Self {
        ...
        let mut app = build_retro_fps_app();
...
mod tests {
    ...
    fn firing_at_a_lined_up_enemy_raises_the_score() {
        ...
        assert!(best > start, "an agent playing retro FPS can score kills");
    }
