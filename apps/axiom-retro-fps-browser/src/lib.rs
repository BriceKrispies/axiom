...
/// Build the engine app for the retro FPS level: the cube-walled rooms, floor and
...
pub fn build_retro_fps_app() -> RunningApp {
    let map = parse_map();
    App::new()
        .window(
            Window::new(960, 600)
...
mod tests {
    ...
    fn the_first_frame_draws_every_renderable_and_runs_deterministically() {
        let mut a = build_retro_fps_app();
        let mut b = build_retro_fps_app();
        let fa = a.tick(0);
