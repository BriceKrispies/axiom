//! App-level deterministic replay proof for the retro FPS game.
//!
//! Drives the **real** deterministic `RetroFpsGame` (not a stand-in) through a fixed
...
#[test]
fn retro_fps_game_replays_byte_identically_through_the_recorder() {
    ...
    assert!(
        report.matched(),
        "retro FPS replay diverged at frame {:?}, artifact {:?}, byte {:?} ({:#x} vs {:#x})",
