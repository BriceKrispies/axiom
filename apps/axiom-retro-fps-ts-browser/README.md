# Axiom — retro FPS, authored purely in TypeScript

A retro FPS-style first-person shooter that is the **TypeScript-only twin of
`apps/axiom-retro-fps-browser`**: the same cube-walled two-room level, first-person
...
## How it differs from `axiom-retro-fps-browser`

| | `axiom-retro-fps-browser` | `axiom-retro-fps-ts-browser` (this app) |
|---|---|---|
| Game logic | Rust (`src/lib.rs` `RetroFpsGame`) | TypeScript (`web/src/game.ts`) |
...
The Rust retro FPS steps its sim **exactly once per displayed frame** (`web.rs`: *"retro FPS
...
- **Mouse-look** flows through the same input pipeline keyboard keys use: a native
  ...
  `Mouse0`. Same path, same `0.0025` rad/px sensitivity as the Rust retro FPS.
- **The camera is the engine's first-person Controller.** `createController` spawns
  ...
  exactly like the Rust retro FPS, which drives the same controller.
...
AXIOM_DEV_APP=axiom-retro-fps-ts-browser node scripts/axiom_dev_server.mjs  # http://localhost:8080
```
