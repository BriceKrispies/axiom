...
// gallery's shared-shell overlay mount, now per-app). wasm-only.
#[cfg(target_arch = "wasm32")]
pub mod overlay;

// The demo's browser entry, re-exported at the crate root — which both names
// the entry and links its `#[wasm_bindgen]` registration into this app's own
// wasm bundle. wasm-only. The `retro_fps_start` export name is kept exactly
// as the gallery exported it.
#[cfg(target_arch = "wasm32")]
