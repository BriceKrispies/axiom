//! Architecture-boundary tests for the `axiom-rotating-cube` app.
//!
//! The demo is a composition leaf: pure scene description on the engine
//! umbrella (`axiom::prelude`), plus the wasm-only overlay/windowing arm. These
//! tests enforce app hygiene — the app imports only what its `app.toml`
//! declares, stays deterministic, and nothing in the engine spine depends back
//! on it.
...
fn app_toml_exists_and_lists_only_consumed_layers_and_modules() {
    ...
    assert!(
        ...
        "app.toml lists the windowing module (the wasm presentation arm)"
    );
    assert!(
        text.contains("\"debug-overlay\""),
        "app.toml lists the debug-overlay module (the wasm overlay mount)"
    );
    // It must not claim modules this demo does not use.
    for forbidden in [
        "\"scene\"",
        "\"render\"",
        "\"webgpu\"",
...
fn no_wall_clock_or_randomness() {
    assert_absent_in_app(
        ...
        "the rotating-cube scene is deterministic: no wall-clock time, no randomness",
    );
}
...
fn no_placeholder_macros() {
    assert_absent_in_app(
        &["todo!", "unimplemented!"],
        "no placeholder architecture in the rotating-cube app",
    );
}

#[test]
fn no_illegal_engine_imports() {
    // The app may import only the engine umbrella (`axiom`, an underscore-free
    // chunk) and, on wasm, the debug overlay facade (+ std / wasm glue).
    ...
    assert!(
        illegal.is_empty(),
        "app may only import the engine umbrella and the debug overlay:\n{}",
        illegal.join("\n")
    );
}
...
fn no_layer_or_module_depends_on_this_app() {
    let mut hits = scan_raw(repo_root().join("crates"), &["axiom_rotating_cube"]);
    ...
    assert!(
        hits.is_empty(),
        "no layer/module may depend on the rotating-cube app:\n{}",
        hits.join("\n")
    );
}
