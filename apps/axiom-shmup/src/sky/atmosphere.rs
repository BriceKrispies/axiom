...
impl Vec3 {
    ...
    /// Componentwise `floor(vec3)`. Added for `super::stars`' `skStarLayer`
    /// (`stars.js:70,76`), which was not a consumer when this `Vec3` was
    /// first ported.
    pub fn floor(self) -> Vec3 {
        Vec3::new(self.x.floor(), self.y.floor(), self.z.floor())
    }

    /// `vec3 + float` (GLSL broadcasts a scalar across every component).
    /// Added for `super::stars`/`super::dome`.
    pub fn add_scalar(self, s: f64) -> Vec3 {
        Vec3::new(self.x + s, self.y + s, self.z + s)
    }

    /// GLSL `mix(a, b, t)` for `vec3`, written as `a + (b - a) * t` — the
    /// same convention as [`gl_mix`] below, so the two never disagree at a
    /// call site that mixes a `Vec3` and an `f64` (e.g. dome.js's ground and
    /// horizon-murk blends).
    pub fn mix(self, o: Vec3, t: f64) -> Vec3 {
        self.add(o.sub(self).scale(t))
    }
}

/// GLSL `mix(a, b, t)`: `a + (b - a) * t`, not `a*(1-t) + b*t` — an
/// arbitrary but *fixed* choice between two algebraically equal forms that
/// round differently in the last bit. Every GLSL-transcribing module in this
/// crate (`dome`, `clouds`, `stars`, `volumetrics`) uses this one function
/// (or [`Vec3::mix`], which is defined the same way) rather than reinventing
/// the formula per call site, which is what keeps the Rust port and
/// `tests/sky/capture.mjs`'s hand-transcription — the only "oracle" that
/// exists for GLSL source with no JS twin — agreeing to the `1e-9` relative
/// tolerance `tests/sky_port.rs` uses throughout.
