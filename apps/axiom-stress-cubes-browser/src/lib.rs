...
//! `#[wasm_bindgen]` `start(cubes)` entry, which authors an `N`-cube grid and
//! `run`s. The engine owns the canvas/GPU binding, the render pipeline, the
