//! Architecture-boundary tests for the `axiom-stress-cubes` app.
//!
//! The app is a composition leaf: its native core is pure scene authoring on
//! the engine umbrella (`axiom`), and only the wasm arm touches the browser
//! (through wasm-bindgen and the debug overlay). These tests pin the app's
//! manifest honesty, its import surface, and its hygiene, and prove no engine
//! layer or module has grown a dependency on this leaf.
...
#[test]
fn app_toml_exists_and_lists_only_consumed_modules() {
    ...
    assert!(
        ...
        "app.toml lists the debug-overlay module (the wasm overlay mount)"
    );
...
fn no_wall_clock_or_randomness() {
    assert_absent_in_app(
        ...
        "the stress scene is deterministic: no wall-clock time, no randomness",
    );
}
...
fn no_placeholder_macros() {
    assert_absent_in_app(
        &["todo!", "unimplemented!"],
        "no placeholder architecture in the stress-cubes app",
    );
}

#[test]
fn no_illegal_engine_imports() {
    // The app may import only the engine umbrella (`axiom`, whose import chunk
    // carries no `axiom_` prefix) and the debug overlay (+ wasm glue crates).
    ...
    assert!(
        illegal.is_empty(),
        "app may only import the engine umbrella and axiom-debug-overlay:\n{}",
        illegal.join("\n")
    );
}
...
fn no_junk_drawer_modules() {
    for path in app_sources() {
        ...
        for banned in ["utils", "helpers", "common", "misc"] {
            assert_ne!(name, banned, "no `{banned}` module in the stress-cubes app");
        }
    }
}
...
fn no_layer_or_module_depends_on_this_app() {
    let mut hits = scan_raw(repo_root().join("crates"), &["axiom_stress_cubes"]);
    ...
    assert!(
        hits.is_empty(),
        "no layer/module may depend on the stress-cubes app:\n{}",
        hits.join("\n")
    );
}
