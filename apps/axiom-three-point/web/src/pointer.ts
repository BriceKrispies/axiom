 ...
 * release-gesture velocity derived from them, ported from the sibling
 * swipe-basketball app (the repo's reference for swipe handling). SDK-free and
 * fully testable. The buffer has a FIXED capacity (`POINTER_HISTORY`) and never
 * grows; a per-tick delta larger than `MAX_POINTER_DELTA` (a tab-switch /
 * lost-focus / missing-sample glitch) is treated as invalid and clears the
 * history so a garbage flick can't be thrown.
 *
 * `releaseVelocity` does NOT just take the last sample pair: it averages the
 * per-pair velocities across the last `SWIPE_SAMPLE_WINDOW` samples with a
 * triangular (middle-emphasised) weighting, so a single jittery final sample — a
 * finger twitch at lift-off — cannot dominate the shot. Samples are canvas
 * pixels (top-left origin, y-down) with the tick they arrived on; the result is
 * pixels-per-tick with screen-down +y (upward swipe ⇒ negative y).
 */

import { type Vec2, vec2 } from "./vec.ts";
import { MAX_POINTER_DELTA, POINTER_HISTORY, SWIPE_SAMPLE_WINDOW } from "./constants.ts";
...
export class PointerHistory {
  ...
  /** Drop all samples (on release, on starting a new gesture, or on a glitch). */
  public clear(): void {
   ...
   * per-pair velocities over the last `SWIPE_SAMPLE_WINDOW` samples. The middle of
  ...
  public releaseVelocity(): Vec2 {
    ...
    const start = Math.max(0, n - (SWIPE_SAMPLE_WINDOW + 1));
