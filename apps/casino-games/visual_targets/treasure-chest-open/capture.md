...
the treasure gem hovering in its glow — toward a polished low-poly reference
...
# 2. capture — the centre chest OPENED, lid up and gem showing, BEFORE the points
...
- **`shot=180`** — freezes the simulation at tick 180 *and* pins the view clock. The
  ...
  window: the chest is open, the gem is up, no score text is on screen.
