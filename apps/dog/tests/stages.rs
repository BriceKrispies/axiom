...
use axiom_dog::{dog_total, headless_animated, DebugView, Dial, SceneConfig, Stage};
...
fn the_study_draws_one_dog_and_no_ground_and_the_field_comes_back_whole() {
    ...
    installed.animate(&mut app, 1, &config, Stage::Study);
    ...
    installed.animate(&mut app, 2, &config, Stage::Field);
    ...
    installed.animate(&mut app, 3, &config, Stage::Study);
    assert_eq!(drawn(&mut app, 3), bones);
    installed.animate(&mut app, 4, &config, Stage::Field);
...
fn the_study_dog_does_not_move_however_far_the_ticks_run() {
    ...
    let posed = |app: &mut axiom::prelude::RunningApp,
                 ...
                 tick: u64| {
        installed.animate(app, tick, &config, Stage::Study);
        app.tick(tick)
            .draws()
            .iter()
            .map(|draw| draw.world())
    ...
    let first = posed(&mut app, &mut installed, 1);
    let much_later = posed(&mut app, &mut installed, 9_000);
    ...
    installed.animate(&mut app, 9_001, &config, Stage::Field);
    let walking = app
        .tick(9_001)
        .draws()
        .iter()
        .map(|d| d.world())
        .collect::<Vec<_>>();
    installed.animate(&mut app, 18_000, &config, Stage::Field);
    let walked = app
        .tick(18_000)
        .draws()
        .iter()
...
fn a_gait_dial_still_re_poses_the_study_but_a_ring_dial_cannot_touch_it() {
    ...
    let standing = study(&mut app, &mut installed, &config, 1);
    ...
    let taller = study(&mut app, &mut installed, &config.with(Dial::LegLength, 1.8), 3);
