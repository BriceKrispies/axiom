//! The typed `EndZoneSettings` model: exactly three settings, each mapping to a
//! real subsystem (documented per-field and in `SETTINGS.md`). Values are
//! bounded by construction, so any loaded or edited state is valid.
...
/// The complete typed settings model — master volume, screen shake, reduced
