...
//!
//! Twelve bodies standing in a row are a demonstration only if you can walk up
//! to one and look at it. This module owns that: a target-relative spherical
//! camera (`yaw`, `pitch`, `distance` about a `target`) that the page's pointer
//! gestures drive and the per-frame closure applies through
//! `RunningApp::set_camera`.
//!
...
///
/// The crucible's scene is small and close: twelve bodies about a unit across,
/// 2.55 apart in two rows of six spanning ~12.8 units of `x` and ~5.3 of depth,
/// on a 34×22 ground plate, framed from 11.15 units out (see
/// [`crate::scene::CAMERA_EYE`]).
///
/// `2.0` is the *inspection* end. At 58° vertical fov a 2.0-unit orbit puts
/// 2.2 world units across the frame's height, so one station's body — the whole
/// point of coming in close — fills the shot, and the eye is still ~0.9 units
/// clear of a body's surface, an order of magnitude outside the 0.1 near plane.
/// Any nearer and the near plane starts eating the sphere you came to look at.
///
/// `60.0` is the *whole-stand* end. The ground plate is the widest thing in the
/// scene at 34 units, and at this fov and the canvas's 2:1 aspect the frame spans
/// `2.216 × distance` in width — so the entire plate is in shot by ~15 units and
/// `60.0` is a comfortable four times that, from any angle, with the far side of
/// the scene still at ~77 units, well inside the 240-unit far plane. It is far
/// enough that the user can never fly off to a scene-less void, and near enough
/// that they can never invert through the target.
...
/// so the zoom feels the same whether you are 2 or 60 units out.
...
impl OrbitState {
    /// Seed the orbit from the authored framing in `scene.rs`, so the opening
...
mod tests {
    ...
    fn the_seed_reproduces_the_authored_framing_exactly() {
        ...
        assert_eq!(state.target(), camera_target());
        // The seed is the authored shot in another coordinate system: square on
        // (no yaw), dead level (no pitch), at the authored stand-off.
        assert!(close(state.yaw(), 0.0));
        assert!(close(state.pitch(), 0.0));
        assert!(state.distance() > MIN_DISTANCE && state.distance() < MAX_DISTANCE);
        // ...and the transform matches the one `scene::camera_transform` authors.
        let authored = crate::scene::camera_transform();
        let live = state.camera_transform();
        assert!(close(live.translation.z, authored.translation.z));
    ...
    fn the_applied_camera_is_the_state_the_gestures_left_behind() {
        ...
        assert_eq!(
            ...
            authored,
            "seeding the orbit moved the opening shot"
        );
