 ...
 */
function bake(size, fn, normalScale = 1) {
 ...
 */
function bakeDetail(size, fn, normalScale = 1) {
  const height = new Float32Array(size * size);
...
export const SOLDIER_SET_NAMES = SOLDIER_SHARDS.flat();

/* ------------------------------------------------------------------ */
/* Public: bake every soldier texture set                              */
/* ------------------------------------------------------------------ */

/**
 * Bake the character texture sets. A seed in, typed arrays out — no THREE, no
 * GPU, no shared state, so this runs unchanged inside a bakery worker.
 *
 * Measured with tools/bootprofile.mjs, this is ~2.5 s and the single largest
 * block of JavaScript in the app's boot. `src/ai/index.js` queues it in
 * `prepare()`, which is why the main thread can spend those seconds building
 * the level instead of waiting for value noise.
 *
 * @param nzSeed  seeds the shared TileNoise. Every set reads from that one
 *   noise object and draws no further randomness, which is what makes the sets
 *   independent of each other and the whole bake reproducible from one number.
 * @returns { sets, details, camoStats, size, bakeMs }, where each set is
 *   `{ albedo, orm, normal }` of Uint8Array and each detail tile is
 *   `{ normal }`. src/ai/textures.js turns them into textures.
 */
export function bakeSoldierSets({ nzSeed, size = 512, camo = ['arid', 'woodland'], only = null }) {
  // SHARDING. Every set below reads the same TileNoise and draws no randomness
  // of its own, so any subset of them can be baked independently and the result
  // is identical to baking them together. `only` names the subset; the caller
  // fans several disjoint shards across the worker pool and merges the results.
  //
  // Rebuilding the noise costs 8192 draws (~1 ms) per shard, which is nothing
  // against the ~300 ms each set takes. Whole-set baking on one worker was
  // ~2.7 s of wall time, and once the rest of boot got fast enough that became
  // the thing `ai.init` was waiting on.
  const want = only ? (name) => only.includes(name) : () => true;
  const nz = new TileNoise(new Rng(nzSeed));
  ...
  const camoStats = {};
  for (const name of camo.filter((c) => want(`camo_${c}`))) {
  ...
  if (want('polymer')) sets.polymer = bake(
    size,
    (u, v, out) => {
  ...
  if (want('rubber')) sets.rubber = bake(
    size,
    (u, v, out) => {
  ...
  if (want('detail_cloth')) details.cloth = bakeDetail(
    dsize,
    (u, v, out) => {
  ...
  if (want('detail_nylon')) details.nylon = bakeDetail(
    dsize,
    (u, v, out) => {
  ...
  const bakeMs = (typeof performance !== 'undefined' ? performance.now() : 0) - t0;
  return { sets, details, camoStats, size, bakeMs };
}
