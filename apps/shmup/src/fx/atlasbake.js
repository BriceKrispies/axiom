/**
 * FX ATLAS BAKING — the pure half: a seed in, pixels out.
 *
 * Every FX texture in this game is baked, once, at load time; there are no
 * image files anywhere in the project. Two atlases:
 *
 ...
 *
 * NOTHING HERE IMPORTS THREE, AND NOTHING HERE MAY. That is the entire point of
 * the split from atlas.js: this module runs unchanged inside a bakery worker
 * (src/core/bakery.js), where there is no WebGL context and no reason to parse a
 * 3D library. It takes a seed and returns typed arrays; atlas.js turns those
 * into textures on the main thread. Measured with tools/bootprofile.mjs, the two
 * atlas bakes are ~1.1 s of the boot's ~9 s of JavaScript.
 *
 * The tile-index tables `P` and `D` live here rather than in atlas.js because
 * the painters below are indexed by them, and seven fx modules import them to
 * pick a sprite. atlas.js re-exports both, so no call site had to change.
 */
...
/* ========================================================================= */
/*  painters — pure, worker-safe, deterministic from a seed                  */
/* ========================================================================= */
...
/**
 * Paint the particle sprite atlas.
 *
 * A SEED, not an `Rng`. The caller draws exactly one `u32()` from its own
 * stream — the draw `rng.fork()` was already making — and passes the number, so
 * the caller's stream advances exactly as it did before AND the job is
 * structured-cloneable into a worker. `new Rng(seed)` reproduces the forked
 * stream bit for bit, which is what keeps the pixel gate honest.
 */
export function paintParticleAtlas({ seed, size = 1024 }) {
  ...
  for (let t = 0; t < 16; t++) {
    ...
    for (let py = 0; py < tile; py++) {
      ...
      for (let px = 0; px < tile; px++) {
        ...
        pixels[i] = encodeSrgb(out[0]) * 255;
        pixels[i + 1] = encodeSrgb(out[1]) * 255;
        pixels[i + 2] = encodeSrgb(out[2]) * 255;
        pixels[i + 3] = clamp01(out[3]) * 255 * gutter;
      }
    }
  }
  return { pixels, cols: ATLAS_COLS, size };
}

/**
 * Paint the decal atlas: albedo+alpha, a normal map derived from the painted
 * height field, and packed ORM.
 */
export function paintDecalAtlas({ seed, size = 1024 }) {
  const n = new Noise(new Rng(seed));
  const tile = size / ATLAS_COLS;
  ...
  return { albedo, normal, orm, cols: ATLAS_COLS, size };
}
 ...
 */
export function paintBrass({ seed, size = 256 }) {
  const n = new Noise(new Rng(seed));
  const normal = new Uint8Array(size * size * 4);
  const orm = new Uint8Array(size * size * 4);
