...
fn field(p: Vec3, caps: &[Capsule], k: f32) -> f32 {
    caps.iter()
        .fold(1.0e9, |acc, &cap| smin(acc, capsule_sdf(p, cap), k))
}
...
fn bounds(caps: &[Capsule], res: u32) -> (Vec3, Vec3) {
    let (lo, hi) = caps.iter().fold(
        (
            Vec3::new(1.0e30, 1.0e30, 1.0e30),
            Vec3::new(-1.0e30, -1.0e30, -1.0e30),
        ),
        |(lo, hi), c| {
            let mn = Vec3::new(
                c.a.x.min(c.b.x) - c.radius,
                c.a.y.min(c.b.y) - c.radius,
                c.a.z.min(c.b.z) - c.radius,
            );
            let mx = Vec3::new(
                c.a.x.max(c.b.x) + c.radius,
...
mod tests {
    ...
    fn bake(params: Vec<Param>) -> Option<MeshBuffer> {
        ...
        ProcCore::new()
            .execute(&g, 7, &SpaceApi::root(), mesh_eval)
            .ok()
    }
    ...
    fn a_single_sphere_bakes_a_closed_surface() {
        ...
        assert_eq!(
            m.indices().len(),
            m.vertex_count(),
            "unwelded: one index per emitted vertex"
        );
    ...
    fn two_overlapping_spheres_fuse_into_one_connected_surface() {
        ...
        assert!(
            m.positions().iter().any(|p| p.x.abs() < 0.1),
            "the two balls are fused, not disjoint"
        );
    }
    ...
    fn metasurface_auto_skins_each_vertex_to_its_capsules() {
        ...
        assert!(m
            .weights()
            .iter()
            .all(|w| (w.iter().sum::<f32>() - 1.0).abs() < 1.0e-4));
        ...
        let dominant_bone = |on_side: &dyn Fn(f32) -> bool| -> u16 {
            let idx = m
                .positions()
                .iter()
                .position(|p| on_side(p.x))
                .expect("a vertex on that side");
    ...
    fn a_capsule_is_longer_than_a_sphere() {
        ...
        assert!(
            y_span > 1.2,
            "capsule spans its segment plus caps, got {y_span}"
        );
    }
    ...
    fn resolution_is_clamped_not_unbounded() {
        ...
        assert_eq!(
            bake(sphere(9999, 0.0, 0.6)),
            bake(sphere(super::MAX_RES, 0.0, 0.6))
        );
