...
const OPS: [TexOp; 11] =
    [solid, gradient, noise, bricks, blur, blend, color_ramp, height_to_normal, checker, text, spots];
...
mod tests {
    ...
    fn unknown_operator_code_fails_the_node() {
        ...
        g.add(200, vec![], vec![]);
        assert!(ProcCore::new().execute(&g, 0, &SpaceApi::root(), texture_eval).is_err());
    }
}
