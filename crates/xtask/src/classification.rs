...
pub fn classify(
    ...
    // Priority-ordered, first match wins (manifest dirs, then well-known
    // package names, then the `tools/` directory).
    index
        .layer_by_dir
        .contains_key(&pkg.dir)
        .then_some(PackageClass::Layer)
        .or_else(|| {
            index
                .module_by_dir
                .contains_key(&pkg.dir)
                .then_some(PackageClass::Module)
        })
        .or_else(|| {
            index
                .app_by_dir
        ...
        .or_else(|| {
            pkg.dir
                ...
                .filter(|first| *first == "tools")
                .map(|_| PackageClass::Tool)
        })
}
