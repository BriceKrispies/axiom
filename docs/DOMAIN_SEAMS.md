...
- **Why not confirmed:** only proven inside individual apps (retro_fps, roomed-puzzle).
...
4. **Per-app input/interaction wiring** (retro_fps, roomed-puzzle, quintet, growth).
...
8. **`scripts/coverage.{retro_32bit,sh}` + `package_app.py` + `ts-gate`.** Coverage gate,
...
Apps (leaves, own all cross-module glue):
  axiom-demo-rotating-cube(+browser), axiom-retro-fps-browser, axiom-stress-cubes-browser,
