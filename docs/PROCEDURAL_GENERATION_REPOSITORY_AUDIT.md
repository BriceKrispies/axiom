...
| `doom-browser-demo` | `axiom-doom-browser` | engine, windowing | FPS gameplay; has `write_state`/`read_state` + replay tests. |
...
- **`modules/axiom-recording/src/determinism_report.rs`** — `DeterminismReport` +
  ...
  `apps/axiom-doom-browser/tests/replay_determinism.rs` (incl. fork-and-resume
...
Secondary early targets: `apps/axiom-doom-browser` (already has
...
- **Phase 1 golden capture:** `apps/axiom-demo-rotating-cube` (already has all
  six artifact boundaries + determinism tests). Fallbacks: `axiom-doom-browser`,
  `quintet`, `roomed-puzzle`.
...
apps/axiom-doom-browser/tests/replay_determinism.rs
