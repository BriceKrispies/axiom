...
- `apps/axiom-doom-browser` — `write_state`/`read_state` game state.
...
- **Crates/modules/apps/tools touched:** `apps/axiom-demo-rotating-cube` (first),
  then `apps/axiom-doom-browser`, `apps/axiom-quintet`,
...
- **Golden tests required:** the captured baselines themselves — at minimum the
  six rotating-cube boundaries for a fixed tick set, the doom state for a fixed
