...
11. **App composition**: player rig (reuse `scene` FPS controller + Doom-style input wiring) standing on streamed chunks; per-frame cull with `math::Frustum`; dynamic chunk meshes via the Phase-0 path.
