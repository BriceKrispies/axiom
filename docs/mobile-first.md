...
> Fast-follows: doom (canvas/HUD/touch-zones from solved rects) and the `interface`
...
adopt touch input** and **doom-browser as the GPU verification target**:
...
2. **`render_size` consumed in the live arm.** The wasm `LiveGpuBinding` now
   ...
   (`modules/axiom-gpu-backend/src/upscale.rs`, wasm32-only). **Verified on doom**
...
3. **Safe-area + orientation.** doom and roomed-puzzle use `viewport-fit=cover` +
...
4. **Device-loss reinitialize.** `present` now surfaces an unrecoverable GPU-loss
   ...
   covered. **Verified on doom** (no regression to the refactored loop). The true
