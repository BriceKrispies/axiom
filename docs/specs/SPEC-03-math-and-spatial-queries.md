...
> Status: Landed (2026-06-30). Native `axiom-math`/`axiom-scene` facade + the full TS projection — the `v2` namespace, the pure predicates (`aabbOverlap`/`pointInRect`/`circleOverlap`), `overlapBox`/`overlapCircle`/`raycast`, and `clamp`/`lerp`/`normalizeAngle` all routed to the authoritative native f32 (the prior `lerp` JS-f64 determinism risk is fixed — it now forwards to `MathApi::lerp`). See the README ledger and [`../reports/SPEC_VS_IMPL_GAP_AUDIT.md`](../reports/SPEC_VS_IMPL_GAP_AUDIT.md).
