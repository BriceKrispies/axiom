...
> Status: Partial (native core + most of the TS projection landed; deferrals below). See
> README footnote ¹ and [`../reports/SPEC_VS_IMPL_GAP_AUDIT.md`](../reports/SPEC_VS_IMPL_GAP_AUDIT.md).
> Landed (native): `axiom-draw2d::Draw2dApi` builds the full neutral draw-list — `rect`/`circle`/`ellipse`/`line`/`path`/`sprite`/`text`+`measure_text`/`linear`+`radial` gradients + the camera/transform stack + the `(layer, submission)` stable sort — and `Draw2dList`/`Draw2dCommand` are **host-owned** (`crates/axiom-host`, the 2D peer of `FramePacket`). The software backend (`axiom-canvas2d-backend::draw2d_raster`) now rasterizes **rect, circle, ellipse (rotation-exact), line, particle-quad, and sprite with per-shape fill + stroke and src-over alpha compositing**.
> Landed (Wave 2.5, TS projection): `@axiom/game`'s `Frame` exposes `tick` + `camera2D`, `rect`, `circle`, `ellipse`, `line` (each with `fill`/`stroke`/`strokeWidth`/`layer`/`alpha`), the particle system, and render targets — every verb forwarding through the `draw2d*` wasm exports of `apps/axiom-game-runtime` to the native builder. `EllipseRadii` bundles `rx`/`ry`/`rotation` so the verb stays within the SDK's ≤3-param budget.
> Deferred (precise): **(1)** backend raster of **path** (`KIND_PATH`, arbitrary polygon scan-fill) and **text glyph-runs** (`KIND_TEXT_GLYPHS`, baked font atlas) + **gradient/paint fills** — their commands are recognised and skipped, never mis-rasterized (`draw2d_raster.rs`). **(2)** TS/runtime exports for **sprite** (raster exists; needs the app-side `loadTexture` upload into the backend's `Draw2dTextures` + a multi-field marshal), **text**/**measureText** (a monospace `measureText` is coverable; a full glyph atlas is the larger piece), **path**, and the **linear/radial gradient** registration verbs. **(3)** **ranged emitter fields** — `EmitterConfig.lifetime`/`speed`/`size` are still scalar end-to-end (the spec's `[min, max]` form awaits a deterministic in-range pick across `ids.rs`/`particles.rs` and the boundary). The clean design for each is to land it bottom-up (native raster/builder → game-runtime export → TS Frame) exactly as Wave 2.5 did for camera/ellipse/line/stroke. Live browser raster is browser-proven (sandbox cannot run browser WebGPU); the GPU backend's `present_draw2d` is still a no-op.
