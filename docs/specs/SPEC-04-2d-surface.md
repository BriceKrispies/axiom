...
> Status: Landed (2026-06-30; residual raster deferrals below). See README footnote ¹ and [`../reports/SPEC_VS_IMPL_GAP_AUDIT.md`](../reports/SPEC_VS_IMPL_GAP_AUDIT.md).
> Landed (native): `axiom-draw2d::Draw2dApi` builds the full neutral draw-list — `rect`/`circle`/`ellipse`/`line`/`path`/`sprite`/`text`+`measure_text`/`linear`+`radial` gradients + the camera/transform stack + the `(layer, submission)` stable sort — and `Draw2dList`/`Draw2dCommand` are **host-owned** (`crates/axiom-host`, the 2D peer of `FramePacket`); plus the §10.2 pure `sample_animation` flip-book sampler. The **software** backend (`axiom-canvas2d-backend`) rasterizes rect / circle / ellipse (rotation-exact) / line / particle-quad / sprite (per-shape fill + stroke, src-over alpha). The **GPU** backend now rasterizes **rect + sprite (alpha, layer sort)** in its wgpu offscreen/`wasm32` platform arm, proven at parity with software (§7 both-backends alpha proof).
> Landed (TS projection): `@axiom/game`'s `Frame` exposes `tick` + `camera2D`/`rect`/`circle`/`ellipse`/`line` (with `fill`/`stroke`/`strokeWidth`/`layer`/`alpha`), the particle system, render targets, and `sampleAnimation`; plus the `measureText`/`loadFont`/`loadTexture` handle seams. The §7 headless marshalled-list proof (real `Frame` → native `Draw2dList` byte layout) is landed.
> Deferred (precise): **(1)** GPU raster of circle / ellipse / line / particle (the software backend has them); **(2)** on **both** backends, raster of **path** (`KIND_PATH`) and **text glyph-runs** (`KIND_TEXT_GLYPHS`) + **gradient/paint fills** — recognised and skipped, never mis-rasterized; **(3)** **ranged emitter fields** (`EmitterConfig.lifetime`/`speed`/`size` are still scalar end-to-end; the `[min, max]` form awaits a deterministic in-range pick). Live browser raster is browser-proven (sandbox cannot run browser WebGPU).
