...
> Status: Landed (2026-06-30; deferrals by decision below). See README footnote ⁵ and [`../reports/SPEC_VS_IMPL_GAP_AUDIT.md`](../reports/SPEC_VS_IMPL_GAP_AUDIT.md).
...
> Landed (2026-06-29, the `@axiom/client` binding): `@axiom/game`'s `NetTransport` seam now binds over `AxiomClient` — `axiomNetFactory`/`netTransportFromClient` (status/localPlayer/sendIntent/onStatus/onRejected/leave) in `packages/axiom-game/src/axiom-net.ts` — plus the `Intent`-derived deterministic flat-record wire codec (`encodeIntent`/`decodeIntent`). The binding is decoupled (an injected `AxiomClientLike`), so it is covered SPINE (not a platform edge). `AxiomClient` gained a public `onRejected(handler)` so a server-rejected intent is observable. **No Rust/wasm net core was built: re-implementing the codec + socket would duplicate `@axiom/client`'s single source of truth, so net binds only at the TS edge and the Rust crates stay the server/native substrate.**
> Landed (2026-06-30): the TS `onSnapshot`/`onRestore` author hooks; the `NetParticipants` authority-snapshot decoder (per-tick `players`/`inputOf`/`joinedThisTick`/`leftThisTick`); the per-player message twin (`ClientIntentFor`/`ServerSnapshotFor` codecs in `@axiom/client`'s `per-player-codec.ts`, byte-identical to the Rust protocol); `hostRoom`/`RoomConfig`/`Room` and `matchmake`/`Match`; the §7 cross-instance determinism golden (two authority instances fed one intent stream are byte-identical) + the byte-parity fixture; and `tools/axiom-netplay-server` now runs the **authored** game headlessly via per-player `RunningApp::tick_with` + the per-player codec (`decode_client_intent_for`), not hard-coded movement.
> Landed (2026-06-30, follow-ups): **delta snapshot encoding** (a sparse byte-patch `ServerSnapshotFor` diff vs the last-acked snapshot, full snapshot the keyframe/fallback; Rust `axiom-net-protocol` + the `@axiom/client` twin + a cross-language byte-parity fixture); **HS256 JWT admission** (hand-built HMAC-SHA256 on `axiom-crypto` + a `tools/axiom-netplay-server` `JwtPolicy` that gates the join); an **unreliable datagram transport** (`@axiom/client` `DatagramTransport` over a `DatagramLink` seam, newest-wins tolerating drop/reorder; live link is the exempt platform edge); and **client-side prediction** — a generic `resimulate` core in Rust `axiom-client-core` and TS `@axiom/client` (`LocalPrediction`), projected as `configureNet({ predictLocalPlayer })`.
> Deferred (by decision): prediction defaults **OFF** and is proven **same-binary** only; **physics net-prediction stays off** because cross-platform deterministic physics (SPEC-10 §17.6) is unresolved — the opt-in flips to default-on with no API change once cross-target f32 determinism is proven.
