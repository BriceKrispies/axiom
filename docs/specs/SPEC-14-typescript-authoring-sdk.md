...
> Status: Landed — the loop, the Sim-level projections, AND the `Scene` shell (filled factories + loop-driven lifecycle) are all real.
> Landed: `@axiom/game` ships `createGame`/`onFixedUpdate`/`onRender`, the pure `stepFrame`/`GameLoop` core, and the **Sim-level** subsystem projections (e.g. `Sim.tweens`, the bridged math/grid/audio surface) backed by `apps/axiom-game-runtime` (`WasmGame`). The `Scene` shell's `this.*` factory namespaces are **filled, not stubs**: `this.add`/`this.physics` are the bridge-backed retained surfaces (SPEC-02/10), `this.input`/`this.time`/`this.tweens` bind to the running tick's snapshot/pump (SPEC-05/07/09), and `this.sound` is the audio surface (SPEC-08) — bound per-tick via `bindFactories` (see `scene.ts`, `scene-runtime.ts`). The `GameLoop` **drives the lifecycle**: `mount(scene)` → `mountScene(...)` and `preload`/`create`/`update` run each tick (`game-loop.ts:16,56,61`). Gate green: tsgo + Oxlint (every category an error + the branch ban) + 100% `node:test`. The one remaining **documented deferral** is `this.cameras` (the 2D camera/view factory, SPEC-04), still a placeholder by decision.
