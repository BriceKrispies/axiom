...
in `apps/` (retro_fps, rotating-cube, netcode-sim) and the `tools/` relay. Data
...
- retro_fps app (35) — ignored by request.
- relay tool (18) — reductions done in working tree, **held**: its `tokio-stream`
  deps tangle `Cargo.lock` with the uncommitted retro_fps agent-bridge WIP.
