...
impl AnimationAuthoringApi {
    ...
    pub fn set_phase_root_motion_move_toward(
        &mut self,
        phase: PhaseId,
        from: &str,
        to: &str,
    ...
    pub fn set_phase_ease_smoothstep(&mut self, phase: PhaseId) -> AuthoringResult<()> {
        self.phase_mut(phase)
            .map(|p| p.set_ease(EaseCurve::SmoothStep))
    }
...
impl AnimationAuthoringApi {
    ...
    pub fn add_set_joint_rotation(
        ...
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::set_joint_rotation(joint, euler)))
    }

    /// Aim `effector` at `target` during `phase`.
    pub fn add_aim_effector_at_target(
        &mut self,
        phase: PhaseId,
        effector: &str,
        target: &str,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::aim_effector_at_target(effector, target)))
    }

    /// Move `effector` a fraction `amount` toward `target` during `phase`.
    pub fn add_move_effector_toward_target(
        &mut self,
        phase: PhaseId,
        effector: &str,
        target: &str,
    ...
    pub fn add_raise_arm_for_balance(
        &mut self,
        phase: PhaseId,
        right: bool,
    ) -> AuthoringResult<()> {
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::raise_arm_for_balance(right)))
    }

    /// Twist the torso toward `target` by `amount` during `phase`.
    pub fn add_torso_twist_toward_target(
        &mut self,
        phase: PhaseId,
        target: &str,
        amount: Ratio,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::torso_twist_toward_target(target, amount.get())))
    }

    /// Draw the right/left leg back by `amount` during `phase`.
    pub fn add_leg_backswing(
        &mut self,
        phase: PhaseId,
        right: bool,
        amount: Ratio,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::leg_backswing(right, amount.get())))
    }

    /// Strike with the right/left leg toward `target` during `phase`.
    pub fn add_leg_strike(
        &mut self,
        phase: PhaseId,
        right: bool,
        target: &str,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::leg_strike(right, target)))
    }

    /// Follow through with the right/left leg toward `target` during `phase`.
    pub fn add_follow_through(
        &mut self,
        phase: PhaseId,
        right: bool,
        target: &str,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_goal(PoseGoal::follow_through(right, target)))
    }
    ...
    pub fn add_run_cycle(
        ...
        self.phase_mut(phase).map(|p| {
            cycle
                .iter()
                .for_each(|&(joint, amplitude, offset, bias, axis)| {
    ...
    pub fn add_pin_effector_to_target(
        &mut self,
        phase: PhaseId,
        effector: &str,
        target: &str,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_constraint(Constraint::pin_effector_to_target(effector, target)))
    }

    /// Keep the gaze on `target` during `phase`.
    pub fn add_keep_gaze_on_target(&mut self, phase: PhaseId, target: &str) -> AuthoringResult<()> {
        self.phase_mut(phase)
            .map(|p| p.push_constraint(Constraint::keep_gaze_on_target(target)))
    }

    /// Keep the center of mass over `support` during `phase`.
    pub fn add_keep_center_of_mass_over_support(
        &mut self,
        phase: PhaseId,
        support: &str,
    ) -> AuthoringResult<()> {
        self.phase_mut(phase)
            .map(|p| p.push_constraint(Constraint::keep_center_of_mass_over_support(support)))
    }

    /// Orient `effector`'s surface toward `target` during `phase`.
    pub fn add_orient_surface_toward_target(
        &mut self,
        phase: PhaseId,
        effector: &str,
        target: &str,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_constraint(Constraint::orient_surface_toward_target(effector, target)))
    }

    /// Preserve `effector`'s contact with `target` during `phase`.
    pub fn add_preserve_foot_contact(
        &mut self,
        phase: PhaseId,
        effector: &str,
        target: &str,
        ...
        self.phase_mut(phase)
            .map(|p| p.push_constraint(Constraint::preserve_foot_contact(effector, target)))
    }
    ...
    pub fn add_contact(
        ...
        self.phase_mut(phase)
            .map(|p| p.push_contact(ContactDeclaration::new(effector, target)))
    }
    ...
    pub fn add_named_event(
        ...
        self.motion_mut(motion)
            .map(|m| m.add_event(MotionEvent::named(tick, name)))
    }
...
impl AnimationAuthoringApi {
    ...
    pub fn compile(&mut self, motion: MotionId) -> AuthoringResult<PlanId> {
        ...
        let built = self.motion(motion).and_then(|spec| {
            self.rig(spec.rig())
                .and_then(|rig| MotionCompiler::compile(spec, rig))
        });
        built.map(|plan| {
    ...
    pub fn motion_style(&self, motion: MotionId, name: &str) -> AuthoringResult<Option<Ratio>> {
        self.motion(motion)
            .map(|m| m.style_value(name).map(Ratio::finite_or_zero))
    }
    ...
    pub fn frame_ball_contact(
        &self,
        frame: &PoseFrame,
    ) -> Option<(EffectorId, TargetId, TargetId, Ratio)> {
...
impl AnimationAuthoringApi {
    ...
    pub fn active_phase_name(&self, plan: PlanId, tick: Tick) -> AuthoringResult<Option<String>> {
        self.plan(plan)
            .map(|p| p.active_phase_name(tick.raw()).map(str::to_string))
    }
    ...
    pub fn plan_effector_id(
        &self,
        plan: PlanId,
        name: &str,
    ) -> AuthoringResult<Option<EffectorId>> {
    ...
    pub fn objective_root_velocity(
        &self,
        plan: PlanId,
        tick: Tick,
    ) -> AuthoringResult<Option<Vec3>> {
        self.plan(plan)
            .map(|p| physical_objective::root_velocity(p, tick.raw()))
    }
    ...
    pub fn objective_foot_plant(
        &self,
        plan: PlanId,
        tick: Tick,
    ) -> AuthoringResult<Option<(EffectorId, Vec3)>> {
        self.plan(plan)
            .map(|p| physical_objective::foot_plant(p, tick.raw()))
    }
    ...
    pub fn objective_joint_motors(
        &self,
        plan: PlanId,
        tick: Tick,
    ) -> AuthoringResult<Vec<(JointId, Vec3, Ratio)>> {
        self.plan(plan)
            .map(|p| physical_objective::joint_motors(p, tick.raw()))
    }
    ...
    pub fn objective_ball_impulse(
        &self,
        plan: PlanId,
        tick: Tick,
    ) -> AuthoringResult<Option<(EffectorId, Vec3, Ratio)>> {
        self.plan(plan)
            .map(|p| physical_objective::ball_impulse(p, tick.raw()))
    }
    ...
    pub fn objective_gaze(&self, plan: PlanId, tick: Tick) -> AuthoringResult<Option<Vec3>> {
        self.plan(plan)
            .map(|p| physical_objective::gaze(p, tick.raw()))
    }
}
