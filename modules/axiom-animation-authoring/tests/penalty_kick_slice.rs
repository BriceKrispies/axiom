...
fn an_unknown_joint_effector_or_target_is_rejected_at_compile() {
    ...
    a.add_set_joint_rotation(ph, "no_such_joint", Vec3::ZERO).unwrap();
    ...
    b.add_aim_effector_at_target(ph, "no_such_effector", "ball").unwrap();
...
fn a_non_finite_authored_value_is_rejected() {
    ...
    let rig = api.standard_humanoid();
    let m = api.create_motion("k", Tick::new(10), rig).unwrap();
    api.add_target(m, "bad", Vec3::new(f32::NAN, 0.0, 0.0)).unwrap();
    assert!(api.compile(m).is_err());
}
...
fn the_approach_phase_moves_the_root_toward_the_ball() {
    ...
    let early = api.frame_root(&api.sample(plan, Tick::new(2)).unwrap()).translation;
    let late = api.frame_root(&api.sample(plan, Tick::new(10)).unwrap()).translation;
...
fn the_backswing_places_the_right_foot_behind_the_body() {
    ...
    assert!(foot.z - root.z < 0.0, "foot {foot:?} not behind root {root:?}");
}
...
fn the_strike_emits_exactly_one_ball_contact() {
    ...
    let ticks_with_contact: Vec<u64> = (0..60)
        .filter(|&t| {
            api.frame_ball_contact(&api.sample(plan, Tick::new(t)).unwrap()).is_some()
        })
        .collect();
    assert_eq!(ticks_with_contact.len(), 1, "exactly one ball_contact expected");
    // It fires during the strike phase span [32, 44).
    let at = ticks_with_contact[0];
    assert!((32..44).contains(&at), "ball_contact at {at} not in the strike phase");
}
...
fn the_follow_through_moves_the_right_foot_past_the_ball_toward_the_net() {
    ...
    assert!(foot.z > 0.0, "foot {foot:?} did not pass the ball toward the net");
}
    ...
    let power_at = |p: f32| {
        ...
        (0..60)
            .find_map(|t| {
                ...
                api.frame_ball_contact(&frame).map(|(_, _, _, power)| power.get())
            })
            .unwrap()
    };
