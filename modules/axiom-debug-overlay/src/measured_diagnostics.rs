//! The measured-diagnostics driver — the `wasm32` arm that mounts the overlay
//! and feeds it real, measured values (frame counter, fps + frame time over a
//! short window, live document visibility, and the probed render backend) every
//! animation frame. Compiled only for `wasm32`, behind the facade, exactly like
//! [`crate::dom_binding`]: it never enters the native build, the coverage gate,
//! or the branchless lint.
//!
//! This drives the *default* overlay experience an app gets for free from
//! [`crate::DebugOverlayApi::mount_with_measured_diagnostics`]. Apps that run a
//! real engine loop and want to push their own richer diagnostics keep using the
//! facade's granular setters instead.
...
pub(crate) fn probe_backend() -> (String, u32, String) {
...
pub(crate) fn drive(overlay: DebugOverlayApi) {
