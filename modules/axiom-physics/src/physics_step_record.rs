...
/// normal impulse); `frictioned_contact_count` is the number that additionally
/// received a tangential friction impulse (approaching, frictional, and sliding);
/// `integration_count` is the number of bodies integrated; and `substep_count` is
/// how many substeps the step was split into (these are summed across substeps).
/// `solver_iteration_count` is the **configured** sequential-impulse iteration
/// count — diagnostic metadata describing the solver budget, **not** a proof that
/// any contact was solved (use `solved_contact_count` for that).
...
impl PhysicsStepRecord {
    ...
    pub(crate) fn new(
        ...
        PhysicsStepRecord {
            ...
            frictioned_contact_count: result.frictioned_contact_count(),
    ...
    /// Contacts that additionally received a tangential friction impulse this
    /// step — approaching contacts with positive combined friction and nonzero
    /// tangential relative speed (summed across substeps). Always `<=`
    /// `solved_contact_count`; zero for a frictionless or non-sliding contact.
    pub fn frictioned_contact_count(&self) -> u32 {
...
mod tests {
    ...
    fn empty_is_all_zero() {
        ...
        assert_eq!(r.frictioned_contact_count(), 0);
    ...
    fn new_exposes_every_count() {
        let result = PhysicsStepResult::new(7, 2, 1, 1, 1, 8, 3);
    ...
    fn derives_are_exercised() {
        let result = PhysicsStepResult::new(7, 0, 0, 0, 0, 8, 1);
        let r = PhysicsStepRecord::new(1, 2, 3, 4, 5, 6, &result);
        let c = r;
        assert_eq!(r, c);
