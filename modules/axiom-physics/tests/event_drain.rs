...
fn drain_events_returns_events_in_order() {
    ...
    assert_eq!(
        view, drained_text,
        "drain returns the same events the view showed"
    );
    assert_eq!(drained.len(), 4);
    assert!(
        drained_text[0].contains("BodyCreated"),
        "first event is a creation"
    );
    assert!(drained_text[1].contains("BodyCreated"));
    assert!(drained_text[2].contains("ColliderAttached"));
    assert!(
        drained_text[3].contains("StepCompleted"),
        "last event is the step completion"
    );
}
...
fn drain_events_clears_internal_event_queue() {
    ...
    assert!(
        api.events().is_empty(),
        "the queue is empty immediately after a drain"
    );
}
...
fn step_completed_does_not_grow_without_bound_when_drained_each_step() {
    ...
    for _ in 0..1000 {
        ...
        assert!(
            api.events().is_empty(),
            "drained each step, the log never grows"
        );
    }
}
...
fn draining_empty_events_is_deterministic() {
    ...
    assert_eq!(
        first, second,
        "draining an empty log is deterministic and empty"
    );
}
