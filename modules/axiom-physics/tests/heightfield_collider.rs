...
fn ball_y(px: &PhysicsApi, ball: PhysicsBodyHandle) -> f32 {
    px.snapshot().bodies().iter().find(|b| b.handle() == ball).unwrap().transform().translation.y
}

fn ball_xz(px: &PhysicsApi, ball: PhysicsBodyHandle) -> (f32, f32) {
    let t = px.snapshot().bodies().iter().find(|b| b.handle() == ball).unwrap().transform().translation;
    (t.x, t.z)
}
...
fn a_sphere_settles_on_a_flat_heightfield() {
    ...
    let ground = px.create_static_body(Transform::from_translation(Vec3::ZERO)).unwrap();
    let heights = vec![m(0.0); 25]; // flat 5×5 grid at local y = 0
    px.attach_heightfield_collider(ground, 5, 5, m(1.0), m(1.0), &heights, mat, false).unwrap();

    let ball = px.create_dynamic_body(Transform::from_translation(Vec3::new(0.0, 3.0, 0.0)), r(1.0)).unwrap();
    ...
    assert!(y > 0.25 && y < 1.0, "ball rests on the flat heightfield, y = {y}");
}
...
fn a_sphere_slides_downhill_on_a_tilted_heightfield() {
    ...
    let ball = px.create_dynamic_body(Transform::from_translation(Vec3::new(0.0, 1.5, 0.0)), r(1.0)).unwrap();
    ...
    assert!(ball_y(&px, ball) > -1.0, "ball stays on the ramp, not through it");
}
...
fn heightfield_attach_validates_grid_dimensions() {
    ...
    let ground = px.create_static_body(Transform::from_translation(Vec3::ZERO)).unwrap();
    // nx < 2 is rejected.
    assert!(px.attach_heightfield_collider(ground, 1, 3, m(1.0), m(1.0), &vec![m(0.0); 3], mat, false).is_err());
    // A heights length that is not nx·nz is rejected.
    assert!(px.attach_heightfield_collider(ground, 3, 3, m(1.0), m(1.0), &vec![m(0.0); 4], mat, false).is_err());
    // A well-formed grid attaches.
    assert!(px.attach_heightfield_collider(ground, 3, 3, m(1.0), m(1.0), &vec![m(0.0); 9], mat, false).is_ok());
}
...
fn heightfield_attach_rejects_a_missing_body_and_over_capacity() {
    ...
    let mut full = PhysicsApi::with_config(Vec3::new(0.0, -9.8, 0.0), 8, 4, 1, 1, true, r(0.0), r(0.0)).unwrap();
    let ground = full.create_static_body(Transform::from_translation(Vec3::ZERO)).unwrap();
    assert!(full.attach_sphere_collider(ground, m(0.5), mat, false).is_ok());
    assert!(
        full.attach_heightfield_collider(ground, 3, 3, m(1.0), m(1.0), &vec![m(0.0); 9], mat, false).is_err(),
