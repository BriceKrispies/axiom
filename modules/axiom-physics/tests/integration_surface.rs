...
fn world_with_one(shape: &str) -> PhysicsApi {
    ...
    match shape {
        "sphere" => {
            api.attach_sphere_collider(body, meters(2.0), mat, false)
                .unwrap();
        }
        "box" => {
            api.attach_box_collider(body, Vec3::new(1.0, 2.0, 3.0), mat, false)
                .unwrap();
        }
        "capsule" => {
            api.attach_capsule_collider(body, meters(0.5), meters(1.5), mat, false)
                .unwrap();
        }
        _ => {
            api.attach_plane_collider(body, Vec3::UNIT_Y, meters(5.0), mat, false)
                .unwrap();
        }
    }
    api
...
fn collider_snapshot_exposes_sphere_radius() {
    ...
    assert!(
        shape.box_half_extents().is_none(),
        "a sphere has no box extents"
    );
}
...
fn collider_snapshot_exposes_box_half_extents() {
    ...
    assert!(
        shape.sphere_radius().is_none(),
        "a box has no sphere radius"
    );
}
...
fn latest_contact_report_exposes_contact_point_and_normal() {
    ...
    api.attach_plane_collider(ground, Vec3::UNIT_Y, meters(0.0), mat, false)
        .unwrap();
    let ball = api
        .create_dynamic_body(
        ...
        .unwrap();
    api.attach_sphere_collider(ball, meters(0.5), mat, false)
        .unwrap();
    ...
    assert_eq!(
        contacts.len(),
        1,
        "the sphere/plane contact must be reported"
    );
...
fn contact_report_order_is_deterministic() {
    ...
    let run = || {
        ...
        let left = api
            .create_dynamic_body(
                Transform::from_translation(Vec3::new(-3.0, 0.4, 0.0)),
                ratio(1.0),
            )
            .unwrap();
        api.attach_sphere_collider(left, meters(0.5), mat, false)
            .unwrap();
        let right = api
            .create_dynamic_body(
                Transform::from_translation(Vec3::new(3.0, 0.4, 0.0)),
            ...
            .unwrap();
        api.attach_sphere_collider(right, meters(0.5), mat, false)
            .unwrap();
        api.step(tenth_second()).unwrap();
        api.latest_contacts()
            .iter()
...
fn integration_surface_does_not_mutate_world() {
    ...
    assert_eq!(
        snap1, snap2,
        "read-only inspection must not mutate the world"
    );
}
...
fn public_callers_can_inspect_invalid_mass_error_code() {
    ...
    assert_eq!(
        err.raw_code(),
        2,
        "InvalidMass has the documented stable discriminant 2"
    );
}
...
fn public_callers_can_inspect_missing_body_error_code() {
    ...
    assert_eq!(
        err.raw_code(),
        5,
        "BodyNotFound has the documented stable discriminant 5"
    );
}
...
fn public_callers_can_inspect_non_dynamic_force_error_code() {
    ...
    assert_eq!(
        err.raw_code(),
        9,
        "ForceOnNonDynamicBody has the documented stable discriminant 9"
    );
}
...
fn error_debug_text_is_not_required_for_logic() {
    ...
    assert_ne!(
        mass_err.raw_code(),
        missing_err.raw_code(),
        "distinct codes distinguish them"
    );
}
