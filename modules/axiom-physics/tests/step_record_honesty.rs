...
fn ball_on_plane(y: f32, substeps: u32, iterations: u32) -> (PhysicsApi, PhysicsBodyHandle) {
...
fn zero_contact_step_reports_zero_solved_contacts() {
    ...
    assert_eq!(
        record.solved_contact_count(),
        0,
        "no contact -> zero solved"
    );
}
...
fn contact_step_reports_nonzero_solved_contacts() {
    ...
    assert_eq!(
        ...
        1,
        "the sphere/plane pair is in contact"
    );
    assert!(
        record.solved_contact_count() >= 1,
        "an approaching contact must be solved"
    );
}
...
fn solver_config_iteration_count_is_metadata_not_work_proof() {
    ...
    assert_eq!(
        ...
        5,
        "the configured budget is reported as metadata"
    );
    assert_eq!(
        record.solved_contact_count(),
        0,
        "but no contact was actually solved"
    );
}
...
fn broad_phase_pair_count_reports_actual_pairs() {
    ...
    assert_eq!(
        ...
        1,
        "plane pairs with the sphere"
    );

    // A lone body forms no pair at all.
    let mut solo = PhysicsApi::new();
    solo.create_dynamic_body(Transform::IDENTITY, ratio(1.0))
        .unwrap();
    solo.step(tenth_second()).unwrap();
    assert_eq!(
        solo.latest_step_record().broad_phase_pair_count(),
        0,
        "a lone body forms no pair"
    );
}
...
fn contact_pair_count_reports_actual_contacts() {
    ...
    assert_eq!(
        touching.latest_step_record().contact_pair_count(),
        1,
        "penetrating -> one contact"
    );
    ...
    assert_eq!(
        ...
        1,
        "still a broad pair (infinite plane)"
    );
    assert_eq!(
        record.contact_pair_count(),
        0,
        "but no narrow-phase contact"
    );
}
...
fn substep_count_reports_actual_substeps() {
    ...
    api.step(tenth_second()).unwrap();
    assert_eq!(
        api.latest_step_record().substep_count(),
        4,
        "the configured substeps are reported"
    );
}
