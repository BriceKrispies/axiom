...
fn max_substeps_is_consumed_by_step() {
    ...
    assert_eq!(
        ...
        5,
        "the configured substeps are run"
    );
}
...
fn max_substeps_one_preserves_existing_behavior() {
    ...
    assert!(
        (vel(&api, body).y - expected_v).abs() < 1.0e-6,
        "vy {}",
        vel(&api, body).y
    );
    assert!(
        (pos(&api, body).y - expected_y).abs() < 1.0e-6,
        "y {}",
        pos(&api, body).y
    );
}
...
fn large_step_is_substepped_and_does_not_tunnel_through_plane() {
    ...
    assert!(
        pos(&coarse, ball_c).y < 0.0,
        "coarse run must tunnel below the plane, y={}",
        pos(&coarse, ball_c).y
    );
...
fn commands_apply_once_across_substeps() {
    ...
    let mut api =
        PhysicsApi::with_config(Vec3::ZERO, 8, 16, 16, 4, true, ratio(0.0), ratio(0.0)).unwrap();
    let body = api
        .create_dynamic_body(Transform::IDENTITY, ratio(1.0))
        .unwrap();
    ...
    assert!(
        (vel(&api, body).x - 10.0).abs() < 1.0e-4,
        "vx {} (impulse applied once)",
        vel(&api, body).x
    );
}
...
fn step_completed_event_emits_once_for_outer_step() {
    ...
    assert_eq!(
        completed, 1,
        "exactly one StepCompleted per outer step, regardless of substeps"
    );
}
...
fn substepped_world_replays_identically() {
    ...
    assert_eq!(
        run(),
        run(),
        "a substepped collision run must replay byte-equal"
    );
}
