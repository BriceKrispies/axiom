...
//! (roadmap Phase 9). [`PlacementApi::scatter`] bakes a two-node scatter recipe —
//! draw `count` words, then reduce each word to an integer grid position — through
//! the shared recipe executor at a content address, producing a reproducible
//! placement: the same `(seed, address, count, bounds)` always yields the same
//! scatter, on every run and platform.
//!
//! ## What it is, and is not
//! - A reusable **engine module** (not app-local): a domain capability apps and
//!   feature modules compose. It depends on the `space` + `recipe` + `proc-core`
//!   (+ `kernel`) layers and on **no other module**.
