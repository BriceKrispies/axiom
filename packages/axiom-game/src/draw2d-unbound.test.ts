...
import type { EmitterConfig } from "./draw2d-binding.ts";
import { UNBOUND_DRAW2D } from "./draw2d-unbound.ts";
...
test("the inert UNBOUND_DRAW2D surface makes every draw verb a safe no-op", () => {
  assert.doesNotThrow(() => {
    ...
    UNBOUND_DRAW2D.draw2dPath([{ x: 0, y: 0 }, { x: 1, y: 1 }], { closed: true, fill: RED });
...
test("the inert UNBOUND_DRAW2D id-returning verbs mint the null handle / empty list", () => {
  ...
  // The gradient paints mint the null paint handle before a host binds.
  assert.equal(UNBOUND_DRAW2D.draw2dLinearGradient({ x: 0, y: 0 }, { x: 1, y: 0 }, [{ color: RED, offset: 0 }]), 0);
  assert.equal(UNBOUND_DRAW2D.draw2dRadialGradient({ x: 0, y: 0 }, 1, [{ color: RED, offset: 0 }]), 0);
