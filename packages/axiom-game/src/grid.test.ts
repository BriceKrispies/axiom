...
test("idx is row-major and inBounds gates the grid rectangle on every edge", () => {
  const grid = createGrid(3, 2, 0);
  assert.equal(grid.idx(2, 1), 5);
  assert.equal(grid.inBounds(2, 1), true);
  assert.equal(grid.inBounds(3, 1), false); // x >= cols
  assert.equal(grid.inBounds(0, 2), false); // y >= rows
  assert.equal(grid.inBounds(-1, 0), false); // x < 0
  assert.equal(grid.inBounds(0, -1), false); // y < 0
});
