...
import { EASES, makeTweens } from "./tweens.ts";
...
test("the ease table is the dense index contract, with linear at index 0", () => {
  assert.equal(EASES[0], "linear");
  assert.equal(EASES.indexOf("quadIn"), 1);
});
