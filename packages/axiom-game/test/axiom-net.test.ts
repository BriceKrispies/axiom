...
} from "../src/axiom-net.ts";
import type { ConnStatus, Intent, JoinConfig } from "../src/net.ts";
...
test("encodeIntent/decodeIntent round-trip every field type, order-independent", () => {
  // A boolean, a number, and a string round-trip exactly.
  ...
  // reorder it): sorted-key encoding makes the bytes independent of insertion order.
...
test("encodeIntent/decodeIntent handle a false boolean and an empty record", () => {
  ...
  // The empty intent encodes to a zero field-count and decodes back to {}.
...
test("localPlayer is absent until admitted, then the client id", () => {
  const fake = new FakeClient();
  const transport = netTransportFromClient(fake);
  // clientId 0 = not yet welcomed -> the absent seat.
  assert.equal(transport.localPlayer(), ([] as PlayerId[]).at(0));
  fake.clientId = 5;
...
test("onStatus forwards the observer and onRejected maps the reason code to text", () => {
  ...
  fake.rejectedObserver(1); // REASON_MALFORMED
  fake.rejectedObserver(99); // out of range -> the unspecified fallback
