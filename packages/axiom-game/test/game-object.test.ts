...
test("add.text / add.rectangle / add.image spawn their render component", () => {
  const fake = new FakeBridge();
...
test("setPosition / setRotation / setScale write the Transform and chain", () => {
  ...
  const returned = object.setPosition(3, 4).setRotation(1.5).setScale(2, 2);
  ...
  // The latest Transform is committed to the native store.
...
test("setVelocity writes a Velocity component and records its values", () => {
  const fake = new FakeBridge();
  const object = makeAdd(fake).sprite("hero", 0, 0);
  object.setVelocity(-2, 6);
  assert.equal(object.vx, -2);
  assert.equal(object.vy, 6);
  const velocity = fake.worldGet(object.entity, "Velocity");
  assert.ok(velocity);
  assert.equal(velocity.kind, "Velocity");
});
