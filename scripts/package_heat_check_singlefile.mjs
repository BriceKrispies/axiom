#!/usr/bin/env node
/*
 * Package the TypeScript-only Heat Check app into ONE self-contained `index.html`
 ...
 * `package_swipe_basketball_singlefile.mjs` (same 3D `boot`+`present3d` path, same
 ...
 *
 * Run:  node scripts/package_heat_check_singlefile.mjs [outfile]
 * Out:  dist/heat-check.html  (default), or the path given.
 */
...
const APP_DIR = join(REPO_ROOT, "apps", "axiom-heat-check");
...
const OUT_FILE = outArg ? join(process.cwd(), outArg) : join(REPO_ROOT, "dist", "heat-check.html");
...
// ---------------------------------------------------------------------------
const work = mkdtempSync(join(tmpdir(), "axiom-heatcheck-"));
try {
