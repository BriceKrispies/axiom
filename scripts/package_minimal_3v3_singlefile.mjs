#!/usr/bin/env node
/*
 * Package the TypeScript-only Minimal 3v3 Basketball app into ONE self-contained
 ...
 *
 * Run:  node scripts/package_minimal_3v3_singlefile.mjs [outfile]
 * Out:  dist/minimal-3v3.html  (default), or the path given.
 */
...
const APP_DIR = join(REPO_ROOT, "apps", "axiom-minimal-3v3");
...
const OUT_FILE = outArg ? join(process.cwd(), outArg) : join(REPO_ROOT, "dist", "minimal-3v3.html");
...
// ---------------------------------------------------------------------------
const work = mkdtempSync(join(tmpdir(), "axiom-min3v3-"));
try {
