#!/usr/bin/env node
/*
 * Package the TypeScript-only Swipe Basketball app into ONE self-contained
 ...
 * (fetched on demand via `npx`) purely as a bundler. A sibling of
 * `package_soccer_penalty_singlefile.mjs` (same 3D `boot`+`present3d` path, same
 ...
 *      ES module. The server-only specifier roots the source imports
 ...
 *
 * Run:  node scripts/package_swipe_basketball_singlefile.mjs [outfile]
 * Out:  dist/swipe-basketball.html  (default), or the path given.
 */
...
const APP_DIR = join(REPO_ROOT, "apps", "axiom-swipe-basketball");
...
const OUT_FILE = outArg ? join(process.cwd(), outArg) : join(REPO_ROOT, "dist", "swipe-basketball.html");
