...
/// True if `span`'s source file is outside the branchless spine — i.e. it lives
/// under a `tests/`, `examples/`, or `benches/` directory (test/support code,
/// exempt like inline `#[cfg(test)]`), OR under `apps/`, `games/`, `tools/`, or
/// the `xtask` crate. The Branchless Law scopes at the app/tooling edge: the
/// reusable spine (`crates/*` layers + `modules/*`) is branchless, but apps, games
/// (the cartridge tier), and tooling are explicitly exempt (see
/// `docs/unbranching.md` and CLAUDE.md's Module Law #14). Spine files never live
/// under any of these directories, so this cannot mask a real spine branch.
fn outside_branchless_spine(cx: &LateContext<'_>, span: Span) -> bool {
    // Resolve through any macro expansion to the outermost call site — a branch
    // generated by a macro (e.g. `tokio::select!` used in a tool) belongs to the
    // file that *invoked* the macro, not the macro's defining crate.
    ...
    path.split('/')
        .any(|c| matches!(c, "tests" | "examples" | "benches" | "apps" | "games" | "tools" | "xtask"))
}
...
impl<'tcx> LateLintPass<'tcx> for EngineNoBranching {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        if is_in_test(cx.tcx, expr.hir_id) || outside_branchless_spine(cx, expr.span) {
            return;
        }
        span_lint_and_help(
            cx,
